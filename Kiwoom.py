...
class Kiwoom(QAxWidget):
    ...
    def _opw00018(self, rqname, trcode):
        total_purchage_price = self._comm_get_data(trcode, "", rqname, 0, "총매입금액")
        total_eval_price = self._comm_get_data(trcode, "", rqname, '0', "총평가금액")
        total_eval_profit_loss_price = self._comm_get_data(trcode, "", rqname, 0, "총평가손익금액")
        total_earning_rate = self._comm_get_data(trcode, "", rqname, 0, "총수익률(%)")
        estimated_deposit = self._comm_get_data(trcode, "", rqname, 0, "추정예탁자산")

        self.opw00018_output['single'].append(Kiwoom.change_format(total_purchage_price))
        ...
        for i in range(rows):
            ...
            purchage_price = self._comm_get_data(trcode, "", rqname, i, "매입가")
            current_price = self._comm_get_data(trcode, "", rqname, i, "현재가")
            eval_profit_loss_price = self._comm_get_data(trcode, "", rqname, i, "평가손익")
            earning_rate = self._comm_get_data(trcode, "", rqname, i, "수익률(%)")

            quantity = Kiwoom.change_format(quantity)
            purchage_price = Kiwoom.change_format(purchage_price)
            current_price = Kiwoom.change_format(current_price)
            eval_profit_loss_price = Kiwoom.change_format(eval_profit_loss_price)
            earning_rate = Kiwoom.change_format(earning_rate)

            self.opw00018_output['multi'].append([name, quantity, purchage_price, current_price, eval_profit_loss_price, earning_rate])

        # print(kiwoom.change_format(total_purchage_price))
...
if __name__ == "__main__":
    ...
    # kiwoom.set_input_value("비밀번호", "gjrenek1")
